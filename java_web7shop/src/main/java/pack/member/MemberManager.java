...
public class MemberManager {
	...
	public ArrayList<ZipcodeDto> zipcodeRead(String dongName){
		...
		try {
			...
			System.out.println("ZipcodeDto err : " + e);
		} finally {
			try {
				if (rs != null)
					rs.close();
	...
	public boolean checkId(String id) {
		...
		try {
			...
			//String sql = "select count(*) from member where id=?";  // 건수(0 or 1)를 찾아 아이디 유무판단
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(sql);
	...
	// 관리자 로그인 처리용
	public boolean adminLoginCheck(String adminid, String adminpwd) {
		boolean b = false;
		try {
